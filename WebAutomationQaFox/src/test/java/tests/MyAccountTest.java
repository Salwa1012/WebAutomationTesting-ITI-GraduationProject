...
public class MyAccountTest extends TestBase {
    ...
    public void UserCannotChangePasswordToSamePassword() {
        myAccountObject = new MyAccountPage(driver);
        myAccountObject.OpenChangePasswordPage();
        myAccountObject.ChangePassword(newpassword, newpassword);
        String actualText = driver.getPageSource();
        Assert.assertFalse(actualText.contains("Success"),
                "User was able to change password to the same one");
    }
    @Test(dependsOnMethods = {"UserCanRegisterSuccessfully"})
    public void UserCannotChangePasswordMissingConfirm() {
        myAccountObject = new MyAccountPage(driver);
        myAccountObject.OpenChangePasswordPage();
        myAccountObject.ChangePasswordMissingConfirm(newpassword);
        String actualText = driver.getPageSource();
        Assert.assertFalse(actualText.contains("Success"),
                "User was able to change password to the same one");
    }
